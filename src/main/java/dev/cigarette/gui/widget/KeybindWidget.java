...
public class KeybindWidget extends BaseWidget<Integer> {
    ...
    public KeybindWidget withDefaultKey(int key) {
        utilKey = InputUtil.fromKeyCode(new KeyInput(key, 0, 0));
    ...
    public boolean keyPressed(KeyInput keyInput) {
        ...
        if (keyInput.key() == GLFW.GLFW_KEY_ESCAPE || keyInput.key() == GLFW.GLFW_KEY_RIGHT_SHIFT) {
            this.setBoundKey(GLFW.GLFW_KEY_UNKNOWN);
        } else {
            this.setBoundKey(keyInput.key());
        }
        clearBinding();
        return true;
    }
